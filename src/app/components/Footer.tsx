'use client'

import Image from 'next/image'
import Link from 'next/link'
import kaiLogo from '../assets/kai-pc.png'

export default function Footer() {
    return (
        <footer className="w-full bg-white before:content-[''] before:block before:w-full before:h-[1px] before:bg-gradient-to-r before:from-[#FF8046] before:to-[#A9DC93]">
            <div className="container mx-auto py-12 px-4">
                <div className="grid grid-cols-1 md:grid-cols-[1fr_3fr_1fr] gap-8">
                    {/* Column 1: Logo only */}
                    <div className="flex flex-col items-center md:items-start">
                        <Image
                            src={kaiLogo}
                            alt="KAI Prevention Center"
                            width={110}
                            height={48}
                            priority
                        />
                    </div>

                    {/* Column 2: Address, Contact, KAI, and Medical Partnership */}
                    <div className="flex flex-col md:flex-row justify-between space-y-8 md:space-y-0">
                        {/* Address and Contact */}
                        <div className="text-xs text-[#7D7D82]">
                            <p>REPUBLICA DO LÍBANO</p>
                            <p>CNPJ: 51.586.591/0001-61</p>
                            <p>AV. REPÚBLICA DO LÍBANO, 1214</p>
                            <p>MOEMA - SÃO PAULO - SP</p>
                            <p>CEP: 04502-001</p>
                            <p className="mt-2">-</p>
                            <p>11-4191-0588</p>
                            <Link href="mailto:contato@kaiprevention.com.br" className="text-[#7D7D82] hover:underline">
                                contato@kaiprevention.com.br
                            </Link>
                        </div>

                        {/* KAI */}
                        <div className="flex flex-row md:gap-16">
                            <ul className="space-y-4 text-sm text-[#7D7D82] flex-1 md:flex-auto">
                                <li><Link href="/" className="text-[#7D7D82] hover:underline">KAI</Link></li>
                                <li><Link href="/" className="text-[#7D7D82] hover:underline">EXPERIÊNCIA KAI</Link></li>
                                <li><Link href="/" className="text-[#7D7D82] hover:underline">HI - SERVIÇOS</Link></li>
                                <li><Link href="/" className="text-[#7D7D82] hover:underline">LONGEVIDADE</Link></li>
                                <li><Link href="/" className="text-[#7D7D82] hover:underline">PARA EMPRESAS</Link></li>
                            </ul>
                        </div>

                        {/* Medical Partnership */}
                        <div className="flex flex-row md:gap-16">
                            <ul className="space-y-4 text-sm text-[#7D7D82] flex-1 md:flex-auto">
                                <li><Link href="/" className="text-[#7D7D82] hover:underline">PARCERIA MÉDICA</Link></li>
                                <li><Link href="/" className="text-[#7D7D82] hover:underline">CONTATO</Link></li>
                                <li><Link href="/" className="text-[#7D7D82] hover:underline">DÚVIDAS FREQUENTES</Link></li>
                                <li><Link href="/" className="text-[#7D7D82] hover:underline">POLÍTICA DE PRIVACIDADE</Link></li>
                            </ul>
                        </div>
                    </div>

                    {/* Column 3: Social Media only */}
                    <div className="flex flex-col items-center md:items-end">
                        <div className="flex mt-2 space-x-4">
                            <Link href="https://instagram.com" aria-label="Instagram" className="text-[#7D7D82] hover:text-gray-700">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                                </svg>
                            </Link>
                            <Link href="https://facebook.com" aria-label="Facebook" className="text-[#7D7D82] hover:text-gray-700">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                                </svg>
                            </Link>
                            <Link href="https://linkedin.com" aria-label="LinkedIn" className="text-[#7D7D82] hover:text-gray-700">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                                    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452z" />
                                </svg>
                            </Link>
                        </div>
                    </div>
                </div>
            </div>
            <div className="bg-gradient-to-r from-[#A9DC93] to-[#FF8046] text-white text-xs py-3 px-6">
                <div className="container mx-auto text-center">
                    © Copyright {new Date().getFullYear()} - Kai Prevention Center. Todos os direitos reservados. O conteúdo deste site não pode ser reproduzido, distribuído, transmitido, ou usado, exceto com a permissão prévia por escrito.
                </div>
            </div>
        </footer>
    )
}
